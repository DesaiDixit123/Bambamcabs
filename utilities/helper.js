...
exports.generateAccessToken = async (userData) => {
    return jwt.sign(userData, process.env.WEB_LOGIN_AUTH_SECRET, {});
};
exports.authenticateToken = async (req, res, next) => {
    ...
    if (typeof bearerHeader !== 'undefined') {
        ...
        jwt.verify(token, process.env.WEB_LOGIN_AUTH_SECRET, (err, auth) => {
            if (err) {
...
exports.generatebookingId = async (bookingid) => {
    let currentDate = new Date();
    let day = currentDate.getDate().toString();
    let month = (currentDate.getMonth() + 1).toString();
    let year = currentDate.getFullYear().toString();
    function rendomgenerate(length){
      var result = '';
      var characters = '0123456789';
      var charactersLength = characters.length;
      for (var i = 0; i < length; i++) {
    ...
    let randomNumber = rendomgenerate(4);
    let resid = 'KC' + randomNumber.toString() + year + day + month + bookingid.toString();
    return resid;
  }