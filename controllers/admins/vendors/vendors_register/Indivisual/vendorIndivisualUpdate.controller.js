...
exports.updateIndividualVendor = async (req, res) => {
  try {
    ...
    if (owner_email && owner_email.toLowerCase() !== vendorData.owner_email.toLowerCase()) {
      let checkEmail = await vendorModel.findOne({ owner_email: owner_email.toLowerCase() }).lean();
    ...
    if (owner_email) updateObj.owner_email = owner_email.trim().toLowerCase();
    ...
    return responseManager.onSuccess("Individual Vendor updated successfully", updatedVendor, res);
